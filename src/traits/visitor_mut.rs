...
use fugue::ir::il::ecode::{UnOp, UnRel, BinOp, BinRel};
...
pub trait VisitMut {
    ...
    fn visit_branch_target_mut(&mut self, branch_target: &mut BranchTarget) {
        match branch_target {
            BranchTarget::Location(ref mut location) => self.visit_branch_target_location_mut(location),
    ...
    fn visit_expr_intrinsic_mut(&mut self, name: &str, args: &mut SmallVec<[Box<Expr>; 4]>, bits: usize) {
    ...
    fn visit_expr_mut(&mut self, expr: &mut Expr) {
        match expr {
            ...
            Expr::BinRel(op, ref mut lexpr, ref mut rexpr) => self.visit_expr_binrel_mut(*op, lexpr, rexpr),
            Expr::BinOp(op, ref mut lexpr, ref mut rexpr) => self.visit_expr_binop_mut(*op, lexpr, rexpr),
            Expr::Cast(ref mut expr, ref mut cast) => self.visit_expr_cast_mut(expr, cast),
            Expr::Load(ref mut expr, size, ref mut space) => self.visit_expr_load_mut(expr, *size, space),
            ...
            Expr::Intrinsic(ref name, ref mut args, bits) => self.visit_expr_intrinsic_mut(name, args, *bits),
    ...
    fn visit_stmt_store_mut(&mut self, loc: &mut Expr, val: &mut Expr, size: usize, space: &mut Arc<AddressSpace>) {
    ...
    fn visit_stmt_mut(&mut self, stmt: &mut Stmt) {
        match stmt {
            ...
            Stmt::Store(ref mut loc, ref mut val, size, ref mut space) => self.visit_stmt_store_mut(loc, val, *size, space),
            Stmt::Branch(ref mut branch_target) => self.visit_stmt_branch_mut(branch_target),
            Stmt::CBranch(ref mut cond, ref mut branch_target) => self.visit_stmt_cbranch_mut(cond, branch_target),
